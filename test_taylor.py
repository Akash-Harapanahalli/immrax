...
import numpy as np
from immrax.taylor.taylor_model import taylor_model_from_function, TaylorModel
...
def test_taylor_from_function():
    ...
    tm = taylor_model_from_function(poly, icentpert(center, radius), max_order)
    ...
    center2 = jnp.array([1.0, 1.0])
    tm2 = taylor_model_from_function(poly, icentpert(center2, radius), max_order)
    ...
    tm_sin = taylor_model_from_function(
        f_sin, icentpert(jnp.array([0.0]), jnp.array([1.0])), 5
    )
...
def plot_taylor_1d(f, tm: TaylorModel, title="Taylor Model vs Function"):
    ...
    ax1.axhline(
        hull.lower[0], color="g", linestyle=":", alpha=0.7, label="Interval hull"
    )
    ...
    ax2.fill_between(
        x,
        tm.remainder.lower[0],
        tm.remainder.upper[0],
        alpha=0.3,
        color="orange",
...
def plot_taylor_comparison():
    ...
    for idx, order in enumerate([1, 3, 5, 7]):
        ax = axes[idx // 2, idx % 2]
        tm = taylor_model_from_function(f_sin, icentpert(center, radius), order)
    ...
    center_2d = jnp.array([0.0, 0.0])
    radius_2d = jnp.array([1.5, np.pi / 2])

    fig = plot_taylor_2d(
        f_2d,
        taylor_model_from_function(f_2d, icentpert(center_2d, radius_2d), 4),
        title="f(x) = x₀² + sin(x₁)",
    )
    ...
    center_exp = jnp.array([0.0])
    radius_exp = jnp.array([2.0])

    tm_exp = taylor_model_from_function(f_exp, icentpert(center_exp, radius_exp), 5)
    ...
    tm_poly = taylor_model_from_function(
        poly, icentpert(jnp.array([0.0, 0.0]), jnp.array([1.0, 1.0])), 3
    )
