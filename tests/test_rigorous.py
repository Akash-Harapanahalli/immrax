...
class TestRigorousKwarg:
    ...
    def test_rigorous_default_false(self):
        """natif should be non-rigorous by default."""
        ...
        default = natif(f)(x)
        explicit_non_rigorous = natif(f, rigorous=False)(x)

        # Default (rigorous=False) should produce identical bounds to explicit False
        assert jnp.allclose(default.lower, explicit_non_rigorous.lower)
        assert jnp.allclose(default.upper, explicit_non_rigorous.upper)
